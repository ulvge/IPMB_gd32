...
void spi_flash_init(void)
{
    ...
    /* SPI2_CLK(PB3), SPI2_MISO(PB4), SPI0_MOSI(PB5) GPIO pin configuration */
    gpio_init(GPIOB, GPIO_MODE_AF_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_3 |GPIO_PIN_5);
    gpio_init(GPIOB, GPIO_MODE_IN_FLOATING, GPIO_OSPEED_50MHZ, GPIO_PIN_4);
    /* SPI2_CS(PE3) GPIO pin configuration */
    ...
    qspi_io23_output_disable(SPI_PERIPH);
...
uint16_t spi_flash_send_halfword(uint16_t half_word)
{
    ...
    spi_i2s_data_transmit(SPI_PERIPH,half_word);
...
void qspi_flash_buffer_read(uint8_t* pbuffer, uint32_t read_addr, uint16_t num_byte_to_read)
{
    ...
    /* enable the qspi */
    qspi_enable(SPI_PERIPH); 
    /* enable the qspi read operation */
    qspi_read_enable(SPI_PERIPH);

...
uint16 LIBIPMI_HL_Set_IPv6Address(IPMI20_SESSION_T *pSession, INT8U *Address,INT8U Prefix,INT8U Channel, int timeout,INT8U flag)
{
    int wRet = 0;
    SetLanConfigReq_T setLANReqBuff;
    SetLanConfigRes_T setLANResBuff;
  ...
  //  setLANReqBuff.ConfigData.IPv6Addrs.IPv6_AddrSrcType=0x80; //Static IP Address Source &th bit Enabled.
    ...
    setLANReqBuff.ConfigData.IPv6Addrs.SetSelector = 0;

    ReqDataLen = 2 + (sizeof(IPv6Addrs_T)-1); 
    wRet = IPMICMD_SetLANConfig(pSession, &setLANReqBuff, ReqDataLen, &setLANResBuff, timeout);
    if(wRet != 0)
    {
        TCRIT("LIBIPMI_HL_SetIPv6Address: Error setting LAN configuration for channel %d\n", Channel);
...
uint16 LIBIPMI_HL_GetI_Pv6Address(IPMI20_SESSION_T *pSession, INT8U *Address,INT8U Channel, int timeout)
{
    int wRet = 0;
    GetLanConfigReq_T getLANReqBuff;
    GetLanConfigRes_T getLANResBuff;
					...
					int timeout)
{
	...
	wRet = LIBIPMI_HL_Get_IPv6Source(pSession, Source, Channel, timeout);
	if (wRet != 0)
	{
		TCRIT ("LIBIPMI, Error getting IPv6 Address Source::%x\n",wRet);
		return wRet;
	...
	wRet = LIBIPMI_HL_GetI_Pv6Address(pSession, Address, Channel, timeout);
	if (wRet != 0)
	{
		TCRIT ("LIBIPMI, Error getting IPv6 Address ::%x\n",wRet);
		return wRet;
...
uint16 LIBIPMI_HL_SetIPv6_NetworkCfg(IPMI20_SESSION_T *pSession,
                                        ...
                                        INT8U Prefix,
                                        INT8U Channel,
                                        int timeout,INT8U flag)
{
        int wRet = 0;
        wRet = LIBIPMI_HL_Set_IPv6Address(pSession, Address,Prefix,Channel, timeout,flag);
        if (wRet != 0)
        {
...
uint16 IPMICMD_GetSerialCfg_SOL_Advanced( IPMI20_SESSION_T *pSession, INT8U* char_acc_interval,INT8U* char_send_threshold,int timeout )
{
	...
	*char_acc_interval = * (((uint8*)(&SolConfigRes))+2);
	*char_send_threshold = * (((uint8*)(&SolConfigRes))+3);

							...
							int timeout)
{
	...
	if (pszUserName)
	{
		strcpy (pSession->hNetworkSession->szUserName, pszUserName);
	
		if( pszPassword )
			strcpy (pSession->hNetworkSession->szPwd, pszPassword);
		else
			strcpy (pSession->hNetworkSession->szPwd,"");
	}
	else	
	{
		strcpy (pSession->hNetworkSession->szUserName,"");
		strcpy (pSession->hNetworkSession->szPwd,"");
	}
								...
								uint8 byConfAlg, uint8 *pbyResPkt, uint32 *pdwIPMIResLen, int timeout)
{
	...
	RSSPPayloadInfo_T		*pPayloadInfo;
	...
	pSession->hNetworkSession->byMsgTag = pOpenSessionReq->MsgTag = (uint8)rand();
	...
	pOpenSessionReq->RemConSessionID = pSession->hNetworkSession->dwRemConSessID = (uint32)rand();
	pPayloadInfo = &pOpenSessionReq->Auth;


	/* Authentication Algorithm */
	memset(pPayloadInfo, 0, sizeof(RSSPPayloadInfo_T) );
	pPayloadInfo->Algorithm = byAuthAlg;
	pPayloadInfo->PayloadType = 0x00;
	pPayloadInfo->PayloadLen  = sizeof (RSSPPayloadInfo_T);

	/* Integrity Algorithm */
	pPayloadInfo++;
	memset(pPayloadInfo, 0, sizeof(RSSPPayloadInfo_T) );
	pPayloadInfo->Algorithm = byIntegrityAlg;
	pPayloadInfo->PayloadType = 0x01;
	pPayloadInfo->PayloadLen  = sizeof (RSSPPayloadInfo_T);

	/* Confidentiality Algorithm */
	pPayloadInfo++;
	memset(pPayloadInfo, 0, sizeof(RSSPPayloadInfo_T) );
	pPayloadInfo->Algorithm = byConfAlg;
	pPayloadInfo->PayloadType = 0x02;
	pPayloadInfo->PayloadLen  = sizeof (RSSPPayloadInfo_T);
							...
							uint32 *pdwIPMIResLen, int timeout)
{
	...
	pRAKPMsg1->MsgTag			=  pSession->hNetworkSession->byMsgTag	= (uint8)rand();
	...
	for(i=0; i<MAX_RAND_NO_LEN; i++)
		pRAKPMsg1->RemConRandomNo[i] = pSession->hNetworkSession->byRemConRandomNo[i] = (uint8)rand();


	strcpy ((char*)pRAKPMsg1->UsrName, pSession->hNetworkSession->szUserName);
...
//  	pRAKPMsg3->StatusCode      = SC_NO_ERROR;
	...
	strcpy ((char*)&Msg3hmac.UsrName[0], pSession->hNetworkSession->szUserName);
								...
								uint32 dwIPMIResLen)
{
	...
	strcpy ((char*)pbyhmac->UsrName, pSession->hNetworkSession->szUserName);
							 ...
							 uint16 wKGLen)
{
	...
	strcpy ((char *)&SIKhmac.UsrName[0], pSession->hNetworkSession->szUserName);

...
{
	IPMI20_SESSION_T	hSession;
	AMISetServiceConfReq_T hAMIGetServiceConfReq;
	int timeout = TIMEOUT;
	int wRet = 0;
	uint32 ServiceID = 0;
	uint8 Privlevel = PRIV_LEVEL_ADMIN;
	...
	memset((char *)&hAMIGetServiceConfReq, 0, sizeof(AMIGetServiceConfRes_T));

	hAMIGetServiceConfReq.ServiceID = ServiceID;
	hAMIGetServiceConfReq.Enable = ServiceConfRequest->CurrentState;
	memcpy(hAMIGetServiceConfReq.InterfaceName, ServiceConfRequest->InterfaceName, MAX_SERVICE_IFACE_NAME_SIZE);
	hAMIGetServiceConfReq.NonSecureAccessPort = ServiceConfRequest->NonSecureAccessPort;
	hAMIGetServiceConfReq.SecureAccessPort = ServiceConfRequest->SecureAccessPort;
	hAMIGetServiceConfReq.SessionInactivityTimeout = ServiceConfRequest->SessionInactivityTimeout;
	hAMIGetServiceConfReq.MaxAllowSession = ServiceConfRequest->MaxAllowSession;
	hAMIGetServiceConfReq.CurrentActiveSession = ServiceConfRequest->CurrentActiveSession;

	wRet = (int)LIBIPMI_HL_AMISetServiceConf(&hSession, &hAMIGetServiceConfReq, timeout);
	if (wRet != 0)
	{

...
AuditLogRec_T AugitRecNo[AUTH_REQ_SERVICES]= 
{
    ...
    {SSH_LOGIN_RECTYPE, SSH_LOGOUT_RECTYPE, SSH_AUTOLOGOUT_RECTYPE, SSH_LOGFAILED_RECTYPE}},
    {KVM_SERVICE_NAME, \
    {KVM_START_RCTYPE, KVM_STOP_RCTYPE, KVM_AUTHFAILED_RCTYPE, KVM_NOPERM_RCTYPE}}
};
#endif
                                                                    ...
                                                                    int timeout )
{
    ...
    *block = GetFWConfRes.FWInfo.IsBlockAll & 0x01;	//Only for IPv4 block status
    //memcpy(block, &GetFWConfRes.FWInfo.IsBlockAll, 1);
...
uint16 IPMICMD_AMIGet_BlockAll_Timeout( IPMI20_SESSION_T *pSession,int *Rulecount, GetTimeout_T *fwtimeout,int timeout )
{
    ...
    for(i=0;i<count;i++)
    {
        ...
        {
            memset(&GetFWConfRes.FWInfo.Info.Entry_info.Block.block,0,1);
...
uint16 IPMICMD_AMIGetAllFirewallRule( IPMI20_SESSION_T *pSession,
                                                                                AllRules *Rules,
                                                                                int timeout )
{
    uint16 wRet;
    ...
    for(i=0;i<count;i++)
    {
        ...
        else if(GetFWConfRes.FWInfo.Info.TYPE == PORT_TIMEOUT )
        {
            Rules->PortRules.type = GetFWConfRes.FWInfo.Info.TYPE;
            Rules->PortRules.pos = i;
            Rules->PortRules.state = GetFWConfRes.FWInfo.Info.State;
        ...
        else if(GetFWConfRes.FWInfo.Info.TYPE == PORT_RANGE_TIMEOUT )
        {
            Rules->PortRules.type = GetFWConfRes.FWInfo.Info.TYPE;
            Rules->PortRules.pos = i;
            Rules->PortRules.state = GetFWConfRes.FWInfo.Info.State;
...
//    FWT Rules;
    ...
    for(i=0;i<count;i++)
    {
        ...
        if( GetFWConfRes.FWInfo.Info.TYPE == PORT )
        {
                                                                    ...
                                                                    int timeout )
{
    ...
    else if(SetRule->type == PORT_TIMEOUT)
    {
            SetFWConfReq.Param = PORT_TIMEOUT;
            SetFWConfReq.CMD_INFO.Port_Data.protocol = SetRule->protocol;
            SetFWConfReq.State = SetRule->state;
            memcpy( &SetFWConfReq.CMD_INFO.Port_Data.Port_NO, &SetRule->port1, 2);
    ...
    else if(SetRule->type == PORT_RANGE_TIMEOUT)
    {
            SetFWConfReq.Param = PORT_RANGE_TIMEOUT;
            SetFWConfReq.CMD_INFO.Port_Data.protocol = SetRule->protocol;
            SetFWConfReq.State = SetRule->state;
            memcpy( &SetFWConfReq.CMD_INFO.Port_Range.starting_port, &SetRule->port1, 2);
            memcpy( &SetFWConfReq.CMD_INFO.Port_Range.closing_port, &SetRule->port2, 2);
                                                                    ...
                                                                    int timeout )
{
    ...
    if(SetRule->type == PORT_RELEASE)
    {
            SetFWConfReq.Param = PORT_RELEASE;
            SetFWConfReq.CMD_INFO.Port_Data.protocol = SetRule->protocol;
            SetFWConfReq.State = SetRule->state;
            memcpy( &SetFWConfReq.CMD_INFO.Port_Data.Port_NO, &SetRule->port1, 2);
            dwReqLen = 2+3;
    }
    else if(SetRule->type == PORT_RANGE_RELEASE)
    {
            SetFWConfReq.Param = PORT_RANGE_RELEASE;
            SetFWConfReq.CMD_INFO.Port_Data.protocol = SetRule->protocol;
            SetFWConfReq.State = SetRule->state;
            memcpy( &SetFWConfReq.CMD_INFO.Port_Range.starting_port, &SetRule->port1, 2);
            memcpy( &SetFWConfReq.CMD_INFO.Port_Range.closing_port, &SetRule->port2, 2);
    ...
    else if(SetRule->type == PORT_TIMEOUT_RELEASE)
    {
            SetFWConfReq.Param = PORT_TIMEOUT_RELEASE;
            SetFWConfReq.CMD_INFO.Port_Data.protocol = SetRule->protocol;
            SetFWConfReq.State = SetRule->state;
            memcpy( &SetFWConfReq.CMD_INFO.Port_Data.Port_NO, &SetRule->port1, 2);
            memcpy( &SetFWConfReq.CMD_INFO.Port_Data.timeout, &SetRule->timeout,12);			
    ...
    else if(SetRule->type == PORT_RANGE_TIMEOUT_RELEASE)
    {
            SetFWConfReq.Param = PORT_RANGE_TIMEOUT_RELEASE;
            SetFWConfReq.CMD_INFO.Port_Data.protocol = SetRule->protocol;
            SetFWConfReq.State = SetRule->state;
            memcpy( &SetFWConfReq.CMD_INFO.Port_Range.starting_port, &SetRule->port1, 2);
            memcpy( &SetFWConfReq.CMD_INFO.Port_Range.closing_port, &SetRule->port2, 2);
	...
	INT8U flushAll, int timeout )
{
	...
	{	//param 1 is used to block IPv4 Address, there is no support for IPv6
		wRet = IPMICMD_AMISET_BlockALL_IPRULES(pSession, 1, blockAll, timeout);
		if(wRet != 0)
		{
                             ...
                             int timeout )
{
	...
	sprintf(smtpServerIP,"%d.%d.%d.%d",ip[0],ip[1],ip[2],ip[3]);
    return( wRet );
}
					...
					int timeout)
{
	...
	strcpy(pSubject, (char *) in_pSubject);
	strcpy(pMsg, (char *)in_pMsg);
		            ...
		            int timeout)
{
	...
	strcpy((char *)SenderAddr,(char *)SmtpInfo->SenderAddr);
	strcpy((char *)Machinename,(char *)SmtpInfo->MachineName);
				...
				int timeout )
{
	...
	if( wRet == 0)
	{
		strncpy(pEmailAddr, emailInfo_res.EMailID, EMAIL_ADDR_SIZE);
		pEmailAddr[EMAIL_ADDR_SIZE - 1] = 0;
	}
				...
				int timeout )
{
	...
	strncpy(emailInfo_req.EMailID, pEmailAddr, EMAIL_ADDR_SIZE);
	emailInfo_req.EMailID[EMAIL_ADDR_SIZE - 1] = 0;
				...
				int timeout )
{
	...
	AMIGetRootUserAccessRes_T    ResGetRootAccess = {0};
					...
					int timeout )
{
	...
	AMISetRootPasswordReq_T    ReqSetRootPswd = {0};
...
uint16 LIBIPMI_HL_GetAllSeviceConf (IPMI20_SESSION_T *pSession, GetServiceInfo_T *GetServiceInfo, int timeout)
{
    ...
    {	
        ...
        strncpy (GetServiceInfo->ServiceInfo [serviceCnt].ServiceName, ServiceNameList [Index], MAX_SERVICE_NAME_SIZE);        
        serviceCnt++;
    }
...
{
    uint16 wRet;
    AMIGetLoginAuditCfgRes_T Res = {0};
...
uint16 LIBIPMI_HL_GetAllIPv6Address(IPMI20_SESSION_T *pSession, INT8U Channel, INT8U *Address,INT8U *AddrCnt, int timeout)
{
    ...
    for (ndx = 0; ndx < 2; ndx ++)
    {
        memset (&AMIGetIPv6AddrRes, 0, sizeof (GetIPv6AddrRes_T));
 ...
 *------------------------------------------------------------------------------------*/
uint8 MapUid_to_SELUid ( int8 *username, char *shell)
{
    struct passwd *pwd = NULL;
    unsigned char retuid = 0xff;
    ...
    else if ( (pwd->pw_uid >= LDAP_FIRST_UID) && (pwd->pw_uid < LDAP_LAST_UID) )
        retuid = LDAP_USER_ID;
    else if ( (pwd->pw_uid >= AD_FIRST_UID) && (pwd->pw_uid < AD_LAST_UID) )
        retuid = AD_USER_ID;
    else if ( (pwd->pw_uid >= RADIUS_FIRST_UID) && (pwd->pw_uid < RADIUS_LAST_UID) )
        retuid = RADIUS_USER_ID;
...
uint16 LIBIPMI_HL_LoginLogoutAudit(IPMI20_SESSION_T *pSession, char *service, char*uname, char *ipstr, uint8 state, uint8 count)
{
    ...
    uid = MapUid_to_SELUid ( (signed char *)uname,NULL);
    if (uid == 0xff)
    {
    ...
    if ( uid == LDAP_USER_ID || uid == AD_USER_ID || uid == RADIUS_USER_ID ) 
    {
         ...
         OEMSELRec.OEMData[1] = MaxUsers + ( (uid == RADIUS_USER_ID) ? 3 : ((uid == LDAP_USER_ID)? 1 : 2)); 
    }
    else
    {
...
static uint16 IPMICMD_AMISetFWProtocol(IPMI20_SESSION_T *pSession,
...
static uint16 IPMICMD_AMIGetFWCfg(IPMI20_SESSION_T *pSession,
...
static uint16 IPMICMD_AMISetFWHostCfg(IPMI20_SESSION_T *pSession,
...
static uint16 IPMICMD_AMISetFWFilePathCfg(IPMI20_SESSION_T *pSession,
		               AMISetFWCfgReq_T* pAMISetFWCfgReq,
		               AMISetFWCfgRes_T* pAMISetFWCfgRes,
                       int timeout)
{
    ...
    dwResLen = sizeof( AMIGetFWCfgRes_T );
    printf("IPMICMD_AMISetFWFilePathCfg() --> Parameter: %d,", pAMISetFWCfgReq->Parameter);
    printf("Size: %d\n", data_size);
    wRet = LIBIPMI_Send_RAW_IPMI2_0_Command( pSession, PAYLOAD_TYPE_IPMI,
                                             ( ( NETFN_AMI << 2 ) + 0x00 ), CMD_AMI_SET_FW_CONFIGURATION,
...
{
    uint16 wRet; 
    AMIGetIPMISessionTimeOutRes_T Res = {0};
                       ...
                       INT8U ReqLen,int timeout)
{
    ...
    wRet = LIBIPMI_Send_RAW_IPMI2_0_Command(pSession, PAYLOAD_TYPE_IPMI,
                                                                                   ( ( NETFN_AMI << 2 ) + 0x00 ),CMD_AMI_DUAL_IMG_SUPPORT,
	...
	char* timezone, int timeout)
{
	...
	strncpy((char*)TimeZoneRes.ZoneName, timezone, TIME_ZONE_LEN);
	...
	char * server_name1, char * server_name2, int status, int timeout)
{
	...
	if( status == AUTO )
	{
		...
		if (strlen(server_name1) != 0)
		{
			...
			strncpy((char*)AMISetNTPCfgReq.ntpconf.Server, server_name1, MAX_SERVER_LEN);
		...
		strncpy((char*)AMISetNTPCfgReq.ntpconf.Server, server_name2, MAX_SERVER_LEN);
		wRet = IPMICMD_AMISetNTPCfg( pSession,&AMISetNTPCfgReq,Reqlen,timeout );
               ...
               INT8U* pReq,INT32U ReqLen, int timeout )
{
    ...
    wRet = LIBIPMI_Send_RAW_IPMI2_0_Command( pSession, PAYLOAD_TYPE_IPMI,
                                         ((NETFN_AMI << 2) + 0x00), CMD_AMI_YAFU_SIGNIMAGEKEY_REPLACE,
                                         pReq,ReqLen,
				...
				int timeout)
{
	...
	{
		wResLen = sizeof(AMIGetSSLCertStatusReq_T)-sizeof(SSLCertInfo_T)+1;
	}
	wRet = LIBIPMI_Send_RAW_IPMI2_0_Command( pSession, PAYLOAD_TYPE_IPMI,
...
uint16 LIBIPMI_HL_AMIGetSSLCertInfo(IPMI20_SESSION_T *pSession,char *CerInfo,int timeout)
{
	...
	strncpy(CerInfo,(char *)GetSSLCertStatusResBuff.SSLCertInfo.CertInfo,strlen((char *)GetSSLCertStatusResBuff.SSLCertInfo.CertInfo));
	return wRet;
}
...
uint16 LIBIPMI_HL_AMIGetSSLPrivateKeyInfo(IPMI20_SESSION_T *pSession,char *PrivateKeyInfo,int timeout)
{
	...
	strncpy(PrivateKeyInfo,(char *)GetSSLCertStatusResBuff.SSLCertInfo.PrivateKeyInfo,strlen((char *)GetSSLCertStatusResBuff.SSLCertInfo.PrivateKeyInfo));
	return wRet;
}
...
{
	uint16 wRet;
	uint32 wResLen = sizeof(AMIGetRemoteKVMCfgRes_T) - 2;
...
{
	AMISetRemoteKVMCfgReq_T SetRemoteKVMCfgReq = {0};
...
uint16 LIBIPMI_HL_AMIGetRemoteKVM_Keyboard_Layout(IPMI20_SESSION_T *pSession,char *kbd_layout,int timeout)
{
	...
	AMIGetRemoteKVMCfgRes_T GetRemoteKVMCfgRes = {0};
	uint16 wRet;
	...
	if(wRet == CC_NORMAL)
	{
		strncpy(kbd_layout,(char *)GetRemoteKVMCfgRes.RemoteKVMCfg.keyboard_layout,KEYBRD_LANG_SIZE);
	}
...
{
	uint16 wRet;
	uint32 wReqLen = sizeof(AMISetRemoteKVMCfgReq_T) -2;
...
uint16 LIBIPMI_HL_AMISetRemoteKVM_Keyboard_Layout(IPMI20_SESSION_T *pSession,char *kbd_layout,int timeout)
{
	...
	strncpy((char *)SetRemoteKVMCfgReq.RemoteKVMcfg.keyboard_layout,kbd_layout,KEYBRD_LANG_SIZE);
	wRet = IPMICMD_AMISetRemoteKVMCfg(pSession,&SetRemoteKVMCfgReq,&SetRemoteKVMCfgRes,timeout);
...
uint16 LIBIPMI_HL_AMIGetRemoteLogConf(IPMI20_SESSION_T *pSession,INT8U *Sysstatus, INT8U *Auditstatus, char *hostname,int timeout)
{
	AMIGetLogConfRes_T GetLogConfRes = {0};
	uint16 wRet;
	...
	*Auditstatus = GetLogConfRes.AuditStatus;
	...
	if (GetLogConfRes.SysStatus == REMOTE_SYSLOG)
		strncpy(hostname,GetLogConfRes.Config.Remote.HostName,sizeof(GetLogConfRes.Config.Remote.HostName));
...
uint16 LIBIPMI_HL_AMISetSysLogConf(IPMI20_SESSION_T *pSession, INT8U Sysenable, INT8U Auditenable, char *hostname, int timeout)
{
	AMISetLogConfReq_T SetLogConfReqBuff = {0};
	uint16 wRet;
	...
	if(Sysenable == REMOTE_SYSLOG)
		strncpy(SetLogConfReqBuff.Config.Remote.HostName,hostname,strlen(hostname));
...
uint16 LIBIPMI_HL_AMIGetVmedia_Encryption(IPMI20_SESSION_T *pSession,INT8U *Media_Encryption, int timeout)
{
	...
	AMIGetVmediaCfgRes_T GetVmediaCfgRes = {0};
	uint16 wRet;
...
{
	AMISetVmediaCfgReq_T SetVmediaCfgReq = {0};
	uint16 wRet;
	...
	for(param = AMI_LDAP_STATUS; param < AMI_LDAP_GROUPROLE_CONFIG;param++)
	{
		...
		if (wRet == CC_SUCCESS)
		{
			...
			case AMI_LDAP_IP_ADDR:
				strcpy(GetldapConfig->IPAddr, GetLDAPConfRes.Config.IPAddr);
				break;
			case AMI_LDAP_IP_ADDR_SEC:
				strcpy(GetldapConfig->IPAddrSecondary, GetLDAPConfRes.Config.IPAddrSecondary);
				break;
			case AMI_LDAP_PASSWORD:
				strcpy(GetldapConfig->Password, GetLDAPConfRes.Config.Password);
				break;
			case AMI_LDAP_BIND_DN:
				strcpy(GetldapConfig->BindDN, GetLDAPConfRes.Config.BindDN);
				break;
			case AMI_LDAP_SEARCH_BASE:
				strcpy(GetldapConfig->SearchBase, GetLDAPConfRes.Config.SearchBase);
				break;
			case AMI_LDAP_ATTRIBUTE_USER_LOGIN:
				strcpy(GetldapConfig->AttributeOfUserLogin, GetLDAPConfRes.Config.AttributeOfUserLogin);
				break;
		...
		for(GroupRole = AMI_LDAP_GROUP_NAME; GroupRole <= AMI_LDAP_GROUP_EXTENDED_PRIVILEGE;GroupRole++)
		{
			...
			if (wRet == CC_SUCCESS)
			{
				...
				case AMI_LDAP_GROUP_NAME:
					strcpy(GetldapPrivConfig[UserID].LDAPRoleGroupNameStr,GetLDAPConfRes.Config.LDAPRoleGroupNameStr);
					break;
				case AMI_LDAP_GROUP_DOMAIN:
					strcpy(GetldapPrivConfig[UserID].LDAPRoleGroupDomainStr,GetLDAPConfRes.Config.LDAPRoleGroupDomainStr);
					break;
...
uint16 LIBIPMI_HL_AMISetLDAPConf(IPMI20_SESSION_T *pSession,IPMILDAPCONFIG * SetldapConfig,int timeout)
{
	...
	uint16 wRet;
	int param = 0,i,Retval[]={3,3,4,4,3,3,49,65,3,9,3};
	INT8U ReqLen=0;
	...
	for(param = AMI_LDAP_STATUS; param < AMI_LDAP_GROUPROLE_CONFIG;param++)
	{
		...
		case AMI_LDAP_PASSWORD:
			...
			{
				strcpy(SetLDAPConfReq.Config.Password,SetldapConfig->Password);
				break;
			}
			else
				continue;
		...
		case AMI_LDAP_BIND_DN:
			...
			{
				strcpy(SetLDAPConfReq.Config.BindDN,SetldapConfig->BindDN);
				break;
			}
			else
				continue;
		...
		case AMI_LDAP_ATTRIBUTE_USER_LOGIN:
			...
			{
				strcpy( SetLDAPConfReq.Config.AttributeOfUserLogin,SetldapConfig->AttributeOfUserLogin);
				break;
			}
			else
				continue;
...
uint16 LIBIPMI_HL_AMIGetRadiusConf(IPMI20_SESSION_T *pSession,IPMIRADIUSCONFIG *radius_config,int timeout)
{
	...
	for(param = AMI_RADIUS_STATUS; param < AMI_RADIUS_VENDOR_DATA;param++)
	{
		...
		if (wRet == CC_SUCCESS)
		{
			...
			case AMI_RADIUS_IP_ADDR:
				strcpy(radius_config->IPAddr, GetRadiusConfRes.Config.IPAddr);
				break;
			case AMI_RADIUS_PORT_NUM:
				radius_config->PortNum=GetRadiusConfRes.Config.PortNum;
				break;
			case AMI_RADIUS_SECRET:
				strcpy(radius_config->Secret,GetRadiusConfRes.Config.Secret);
				break;
...
uint16 LIBIPMI_HL_AMIGetRadiusprivConf(IPMI20_SESSION_T *pSession,IPMIRADIUSPRIVCONFIG radius_priv_config[],int timeout)
{
	...
	for(Accesslevel = 0 ; Accesslevel < MAX_ACCESS_LEVEL_USERS; Accesslevel++)
	{
		...
		if(wRet == CC_SUCCESS)
		{
			...
			strcpy((char*)radius_priv_config[Accesslevel].VendorData,GetRadiusConfRes.Config.Get_Vendordata);
		}
	}
	return wRet;
}
uint16 LIBIPMI_HL_AMISetRadiusConf(IPMI20_SESSION_T *pSession,IPMIRADIUSCONFIG *radius_config,int timeout)
{
	AMISetRadiusReq_T SetRadiusConfReq;
	AMISetRadiusRes_T SetRadiusConfRes;
	int param = 0,i,ReqLenght[]={3,3,4,33,6,6,6};
	INT8U ReqLen=0;
	uint16 wRet;
	for(param = AMI_RADIUS_STATUS; param < AMI_RADIUS_VENDOR_DATA;param++)
	{
		...
		switch(param)
		{
			...
			case AMI_RADIUS_PORT_NUM:
				...
				break;
			case AMI_RADIUS_SECRET:
				strcpy(SetRadiusConfReq.ConfigData.Secret,radius_config->Secret);
				break;
			case AMI_RADIUS_TIMEOUT:
...
uint16 LIBIPMI_HL_AMISetRadiusprivConf(IPMI20_SESSION_T *pSession,IPMIRADIUSPRIVCONFIG radius_priv_config[],int timeout)
{
	...
	for(Accesslevel=0;Accesslevel<MAX_ACCESS_LEVEL_USERS;Accesslevel++)
	{
		for(i=0;i<MAX_MSG_BLOCKS-2;i++)
		{
			...
			strncpy((char*)SetRadiusConfReq.ConfigData.VendorData.VendorData,(char*)radius_priv_config[Accesslevel].VendorData+(i*MAX_BLOCK_DATA_SIZE),MAX_BLOCK_DATA_SIZE);
			if(i==0)
				ReqLen=MAX_BLOCK_DATA_SIZE+3;
			else
				ReqLen=MAX_BLOCK_DATA_SIZE+2;
	...
	for(param = AMI_AD_ENABLE; param < AMI_AD_GROUPROLE_CONFIG;param++)
	{
		...
		if (wRet == CC_SUCCESS)
		{
			...
			case AMI_AD_RAC_DOMAIN:
				strcpy((char*)ad_config->ADRACDomainStr,(char*)GetADConfRes.Config.ADRACDomainStr);
				break;
			case AMI_AD_TYPE:
				ad_config->ADType=GetADConfRes.Config.ADType;
				break;
			case AMI_AD_DC_FILTER1:
				strcpy((char*)ad_config->ADDCFilter1,(char*)GetADConfRes.Config.ADDCFilter1);
				break;
			case AMI_AD_DC_FILTER2:
				strcpy((char*)ad_config->ADDCFilter2,(char*)GetADConfRes.Config.ADDCFilter2);
				break;
			case AMI_AD_DC_FILTER3:
				strcpy((char*)ad_config->ADDCFilter3,(char*)GetADConfRes.Config.ADDCFilter3);
				break;
			case AMI_AD_RAC_USERNAME:
				strcpy((char*)ad_config->ADRACUserName,(char*)GetADConfRes.Config.ADRACUserName);
				break;
			case AMI_AD_RAC_PASSWORD:
				strcpy((char*)ad_config->ADRACPassword,(char*)GetADConfRes.Config.ADRACPassword);
				break;
			}
		}
	}
		...
		for(Role=AMI_AD_GROUP_NAME;Role<=AMI_AD_GROUP_EXTENDED_PRIVILEGE;Role++)
		{
			...
			if(wRet == CC_SUCCESS)
			{
				...
				case AMI_AD_GROUP_NAME:
					 strcpy((char*)ad_priv_config[UserID].SSADRoleGroupNameStr,(char*)GetADConfRes.Config.SSADRoleGroupNameStr);
					 break;
				case AMI_AD_GROUP_DOMAIN:
					 strcpy((char*)ad_priv_config[UserID].SSADRoleGroupDomainStr,(char*)GetADConfRes.Config.SSADRoleGroupDomainStr);
					 break;
	...
	for(param = AMI_AD_ENABLE; param < AMI_AD_GROUPROLE_CONFIG;param++)
	{
		...
		case AMI_AD_DC_FILTER3:
			if(ad_config->ADDCFilter3Len!=0)
			{
				...
				continue;
		case AMI_AD_RAC_USERNAME:
			strcpy((char*)SetADConfReq.Configdata.ADRACUserName,(char*)ad_config->ADRACUserName);
			break;
		case AMI_AD_RAC_PASSWORD:
			SetADConfReq.Configdata.Progressbit=1;
			wRet = IPMICMD_AMISetADConf(pSession, &SetADConfReq, &SetADConfRes,ReqLen, timeout);
	...
	for(param = AMI_VIDEO_RCD_REMOTE_PATH_SUPPORT; param <= AMI_VIDEO_RCD_MNT_SUCCESS;param++)
	{
		...
		if(wRet==CC_SUCCESS)
		{
			...
			case AMI_VIDEO_RCD_PATH:
				strncpy(video_rcd_config->path,GetVideoRcdConfRes.Config.path,MAX_IMAGE_PATH_SIZE);
				break;
			...
			case AMI_VIDEO_RCD_IP_ADDR:
				strncpy(video_rcd_config->ip_addr,GetVideoRcdConfRes.Config.ip_addr,MAX_IP_ADDR_LEN);
				break;
			case AMI_VIDEO_RCD_USERNAME:
				strncpy(video_rcd_config->username,GetVideoRcdConfRes.Config.username,MAX_RMS_USERNAME_LEN);
				break;
			case AMI_VIDEO_RCD_PASSWORD:
				strncpy(video_rcd_config->password,GetVideoRcdConfRes.Config.password,MAX_RMS_PASSWORD_LEN);
				break;
			case AMI_VIDEO_RCD_SHARE_TYPE:
				strncpy(video_rcd_config->share_type,GetVideoRcdConfRes.Config.share_type,MAX_SHARE_TYPE_LEN);
				break;
			case AMI_VIDEO_RCD_DOMAIN:
				strncpy(video_rcd_config->domain,GetVideoRcdConfRes.Config.domain,MAX_DOMAIN_LEN);
				break;
...
uint16 LIBIPMI_HL_AMISetVideoRcdConf(IPMI20_SESSION_T *pSession,IPMI_AutoRecordCfg_T *video_rcd_config,int timeout)
{
	...
	AMISetVideoRcdRes_T SetVideoRcdConfRes;
	int i,param,ReqLenght[]={6,3,6,6,6,64,3,33,7,3,3,6,3};
	uint16 wRet;
	INT8U ReqLen;
	int DomainLen = 0;
	for(param = AMI_VIDEO_RCD_REMOTE_PATH_SUPPORT; param <= AMI_VIDEO_RCD_RST;param++)
	{
		...
		case AMI_VIDEO_RCD_PATH:
			...
			ReqLen=ReqLenght[param];
			break;
		case AMI_VIDEO_RCD_MAX_TIME://Readonly
			continue;
		case AMI_VIDEO_RCD_MAX_SIZE://Readonly
			continue;
		case AMI_VIDEO_RCD_MAX_DUMP://Readonly
			continue;
		case AMI_VIDEO_RCD_IP_ADDR:
			if(strlen(video_rcd_config->ip_addr)!=0)
			{
				strncpy(SetVideoRcdConfReq.Config.ip_addr,video_rcd_config->ip_addr,MAX_IP_ADDR_LEN);
				break;
			}
			else
				continue;
		case AMI_VIDEO_RCD_USERNAME:
			...
			break;
		case AMI_VIDEO_RCD_PASSWORD:
			strncpy(SetVideoRcdConfReq.Config.password,video_rcd_config->password,MAX_RMS_PASSWORD_LEN);
			break;
		case AMI_VIDEO_RCD_SHARE_TYPE:
			strncpy(SetVideoRcdConfReq.Config.share_type,video_rcd_config->share_type,MAX_SHARE_TYPE_LEN);
			break;
		case AMI_VIDEO_RCD_DOMAIN:
...
uint16 LIBIPMI_HL_AMIGetRISCfg(IPMI20_SESSION_T *pSession, INT8U MediaType, MediaConfig_T *rmediacfg,int timeout)
{
	...
	for(Param=RIS_IMAGE_NAME;Param <= RIS_ERR_CODE;Param++)
	{
		...
		{
		case RIS_IMAGE_NAME:
			strncpy(rmediacfg->Image_Name,(char *)GetRISConfResBuff.Config.ImageName,strlen((char *)GetRISConfResBuff.Config.ImageName));
			break;
		case RIS_REMOTE_PATH:
			strncpy(rmediacfg->Image_Path,(char *)GetRISConfResBuff.Config.MountPath,strlen((char *)GetRISConfResBuff.Config.MountPath));
			break;
		case RIS_IP_ADDR:
			strncpy(rmediacfg->Ip_Addr,(char *)GetRISConfResBuff.Config.RemoteIP,strlen((char *)GetRISConfResBuff.Config.RemoteIP));
			break;
		case RIS_USER_NAME:
			strncpy(rmediacfg->Username,(char *)GetRISConfResBuff.Config.UserName,strlen((char *)GetRISConfResBuff.Config.UserName));
			break;
		case RIS_PASSWORD:
			strncpy(rmediacfg->Password,(char *)GetRISConfResBuff.Config.Password,strlen((char *)GetRISConfResBuff.Config.Password));
			break;
		case RIS_SHARE_TYPE:
			strncpy(rmediacfg->Share_Type,(char *)GetRISConfResBuff.Config.ShareType,strlen((char *)GetRISConfResBuff.Config.ShareType));
			break;
		case RIS_DOMAIN:
			strncpy(rmediacfg->Domain, (char *)GetRISConfResBuff.Config.Domain,strlen((char *)GetRISConfResBuff.Config.Domain));
			break;
...
{
	uint16 wRet;
	AMISetRISConfReq_T SetRISConfReqBuff = {0};
	AMISetRISConfRes_T SetRISConfResBuff = {0};
...
uint16 LIBIPMI_HL_AMISetRISImageName(IPMI20_SESSION_T *pSession, INT8U MediaType, INT8U *Image_Name,int timeout)
{
	...
	if (wRet != CC_SUCCESS)
		return wRet;
	for(i=0;(i < MAX_MSG_BLOCKS)&&((i*MAX_BLOCK_DATA_SIZE)<strlen((char *)Image_Name));i++)
	{
...
uint16 LIBIPMI_HL_AMISetRISCfg(IPMI20_SESSION_T *pSession, INT8U MediaType, MediaConfig_T *rmediacfg, INT8U Restart, int timeout)
{
	...
	{ 
		...
		case RIS_REMOTE_PATH:
			...
			if(wRet != CC_SUCCESS)
			{
				...
				continue;
		case RIS_IP_ADDR:
			strncpy(SetRISConfReqBuff.config.RemoteIP,rmediacfg->Ip_Addr,strlen(rmediacfg->Ip_Addr));
			break;
		case RIS_USER_NAME:
			if(strcmp(rmediacfg->Share_Type, "nfs") == 0)
				continue;
		...
		case RIS_PASSWORD:
			if(strcmp(rmediacfg->Share_Type, "nfs") == 0)
				continue;
			strncpy(SetRISConfReqBuff.config.Password,rmediacfg->Password,strlen(rmediacfg->Password));
			break;
		case RIS_SHARE_TYPE:
			strncpy(SetRISConfReqBuff.config.ShareType,rmediacfg->Share_Type,strlen(rmediacfg->Share_Type));
			break;
		case RIS_DOMAIN:
			if(strcmp(rmediacfg->Share_Type, "nfs") == 0)
				continue;
	...
	memset(&SetRISConfReqBuff,0,sizeof(AMISetRISConfReq_T));
	memset(&SetRISConfResBuff,0,sizeof(AMISetRISConfRes_T));
	SetRISConfReqBuff.MediaType = 0x08;
...
{
	AMIRISStartStopReq_T RISStartStopReqBuff = {0};
...
uint16  IPMICMD_AMIGetFeatureStatus(IPMI20_SESSION_T *pSession,	INT8U *FeatureName, INT8U *FeatureStatus,int timeout)
{
    ...
    strcpy((char *)ReqBuf,(char *)FeatureName);
    
    memset(&GetFeatureStatusRes, 0x0, sizeof(AMIGetFeatureStatusRes_T));
    wRet = LIBIPMI_Send_RAW_IPMI2_0_Command( pSession, PAYLOAD_TYPE_IPMI,
                                             ((NETFN_AMI << 2) + 0x00), CMD_AMI_GET_FEATURE_STATUS,
...
{
	uint16 wRet;
	INT8U pReq,pRes[MSG_PAYLOAD_SIZE];
	uint32 ReqLen =1,ResLen;
	int Shrlength,offset,FleNameLth;
	...
	strncpy((char *)FWRemoteImg->UserName,(char *)pRes+2,16);
	//strncpy((char *)FWRemoteImg->Pswd,(char *)pRes+18,16);
	Shrlength = pRes[18];
	strncpy((char *)FWRemoteImg->ShareName,(char *)pRes+19,Shrlength);
	offset = 18+Shrlength+1;
	FleNameLth = pRes[offset];
	strncpy((char *)FWRemoteImg->FileName,(char *)pRes+offset+1,FleNameLth);
	offset= offset+FleNameLth+1;
	
	sprintf((char *)FWRemoteImg->IpAddr ,"%d.%d.%d.%d",pRes[offset],pRes[offset+1],pRes[offset+2],pRes[offset+3]);
...
uint16 LIBIPMI_HL_AMIGetBackupList(IPMI20_SESSION_T *pSession, INT8U Selector, GetBackupList_T *pBackupConfig, int timeout)
{
    ...
    strncpy(((char*)pBackupConfig->ConfigFile), ((char*)Res.GetBackupConfig.GetBackupList.ConfigFile), strlen(((char*)Res.GetBackupConfig.GetBackupList.ConfigFile)));
...
uint16 LIBIPMI_HL_AMIGetPendStatus(IPMI20_SESSION_T *pSession, INT8U *pStatus, INT16U *pErrorCode, int timeout)
{
    ...
    AMIGetPendStatusRes_T Res = {0};

    memset (&Req, 0, sizeof(AMIGetPendStatusReq_T));
    Req.Operation = 38; //PendTask command no to get Backup/Restore config file status
...
{
    AMIMediaRedirctionStartStopReq_T AMIMediaRedirctionStartStopReq = {0};
    uint16 wRet;
    if (ImageName == NULL)
    {
    ...
    else
    {
        ...
        strncpy((char*)AMIMediaRedirctionStartStopReq.ImageInfo.Name,(char *)ImageName,strlen((char *)ImageName));
...
{
   AMIGetMediaInfoReq_T AMIGetMediaInfoReq = {0};
   AMIGetMediaInfoRes_T AMIGetMediaInfoRes = {0};
   uint16 wRet;
...
{
   AMISetMediaInfoReq_T AMISetMediaInfoReq = {0};
   uint16 wRet = 0;
   ...
   strncpy((char *)AMISetMediaInfoReq.Ops.Delete.ImageName, (char *)ImageName,REDIRECTED_IMAGE_LEN);
...
{
	AMIGetUSBSwitchSettingRes_T GetUSBSwitchSettingRes={0};
	uint16 wRet;

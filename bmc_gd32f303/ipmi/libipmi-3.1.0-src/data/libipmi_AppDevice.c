...
{
	uint16 wRet;
	GetChAccessReq_T ReqGetChAccess = {0};
...
{
	uint16 wRet;
	SetChAccessReq_T ReqSetChAccess = {0};
	...
	for(i=1;i<=*nNumUsers;i++)
	{
			...
			if(wRet == 0)
			{
				...
				if(pResGetUserName[i].UserName[0] == 0)
				{
					if(i == 1)
					{
						strcpy((char*)pResGetUserName[i].UserName,(char*)"anonymous");
					}
				}
			}
			else
	...
	INT8 nSerialChNo = 0;
        ...
        if(wRet == 0)
        {
            ...
            if(pResGetUserName->UserName[0] == 0)
            {
                if(UserID == 1)
                {
                    strcpy((char*)pResGetUserName->UserName,(char*)"anonymous");
                }
            }
        }
        else
...
int timeout)
{
	...
	SetUserNameReq_T ReqSetUserName = {0};
	SetUserNameRes_T ResSetUserName = {0};
	SetUserPswdReq_T ReqSetUserPswd = {0};
	SetUserPswdRes_T ResSetUserPswd = {0};
	SetUserAccessReq_T ReqSetUserAccess = {0};
	SetUserAccessRes_T ResSetUserAccess = {0};
	SetUsrPayldAccReq_T  ReqSetUserPayloadAccess ={0};
...
uint16 LIBIPMI_HL_GetUser( IPMI20_SESSION_T *pSession, char *pszUsername, uint8 byID, int timeout )
{
	...
	if(wRet == LIBIPMI_E_SUCCESS)
	{
		strcpy((char*)pszUsername, (char*)ResGetUserInfo.UserName);

                             ...
                             char *pUserName, uint8 *ipaddr, int timeout, int AuthFlag, INT8U ChannelNum)
{
    ...
    if(strlen(pszUserName)!=0)
    {
        strcpy(pSession->hSMBMCUDSSession->szUserName,pszUserName);
    }
    if(strlen(pszPassword)!=0)
    {
        strcpy(pSession->hSMBMCUDSSession->szPwd,pszPassword);
    }

    if( (NULL != pUserName) && (0 != strlen(pUserName)) && (strlen(pUserName) < MAX_USER_NAME_LEN) )
    {
        strcpy(pSession->hSMBMCUDSSession->szUName, pUserName);
    }
    else
    {
        strcpy(pSession->hSMBMCUDSSession->szUName, "");
    }
    if(NULL != ipaddr)
    {

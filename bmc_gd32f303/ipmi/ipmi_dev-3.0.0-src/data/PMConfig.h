...
#define DUAL_IMAGE_FILE(Instance, filename) \
sprintf(filename, "%s%d/%s", NV_DIR_PATH, Instance, "DualImageCfg.ini");

#define THIRDIPMBCHCONFIG_FILE(Instance,filename)   \
sprintf(filename,"%s%d/%s",NV_DIR_PATH,Instance, "ThirdIpmbChCfg.ini");

#define IPMI_CONFIG_VERSION     19
...
typedef struct
{
   ...
   INT8U SpecDateTime;
   INT32U Time;
   //Signal to enable/disable pre crash/boot recording
   INT8U PreLPCRcdFlag;
   INT8U PreBSODRcdFlag;
...
typedef struct
{
    char  EncryptedPswd[MAX_ENCRYPTED_PSWD_LEN + 1]; /* +1 to store null character*/
    char  EncryptedHexPswd[MAX_ENCRYPTED_PSWD_LEN + MAX_ENCRYPTED_PSWD_LEN + 1]; /* +1 to store null character*/
} PACKED EncryptedUserInfo_T;

typedef struct
{
    INT8U FwUploadSelector;
} PACKED DualImageCfg_T;
#pragma pack( )

 ...
 *		-1	if set failed.
 *-----------------------------------------------------------------*/
extern SensorInfo_T* API_GetSensorInfo (INT8U SensorNum, INT8U OwnerLUN, int BMCInst);
...
void API_SensorAverage(INT8U SensorNum, INT8U OwnerLUN, INT16U* SensorReading, int BMCInst);
...
/*-----------------------------------------------------------------
 * @fn API_SetSensorReadingEvtStat  
 * @brief Set the sensor Reading and Event Status
 * @param - SetSensorReadingReq_T SensorReading struct  
 * @return  0 if success 
 *	   -1 if failed
 *-----------------------------------------------------------------*/
extern int API_SetSensorReadingEvtStat(SetSensorReadingReq_T * SensorReading, int BMCInst);

/*-----------------------------------------------------------------
 * @fn API_SetSensorThresholds
 * @brief Sets the sensor thresholds for a particular sensor number
 * @param SensorNum - Sensor Number to look up
 * @param SensorThresh - Structure to store sensor thresholds in
 * @return      0   if success
 *                      -1      if failed.
 *-----------------------------------------------------------------*/
extern int API_SetSensorThresholds (SetSensorThresholdReq_T* SensorThresh, int BMCInst);

/*-----------------------------------------------------------------
 * @fn API_SetSensorReading
 * @brief Sets the sensor reading for a particular sensor number
 * @param SensorNum - Sensor Number to look up
 * @return  0, on success
 *          -1, if failed.
 *-----------------------------------------------------------------*/
extern int API_SetSensorReading (SetSensorReadingReq_T* SensorRead, int BMCInst);
